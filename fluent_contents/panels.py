...
class ContentPluginPanel(Panel):
    ...
    @property
    def title(self):
        return _("{0} content items rendered in {1} placeholders").format(
    ...
    def process_response(self, request, response):
        ...
        for resulttracker in collector.get_collection():
                ...
                try:
                    ...
                    try:
                        content_type = ContentType.objects.get_for_id(
                            contentitem.polymorphic_ctype_id
                        )
                    ...
                    rendered_items.append(
                        {
                            ...
                            "cache_timeout_str": str(timedelta(seconds=cache_timeout))
                            if cache_timeout
                            else None,
                        }
                    )
            ...
            all_timeout = (
                None
                if isinstance(resulttracker.all_timeout, object)
                else int(resulttracker.all_timeout)
            )
            ...
            rendered_placeholders.append(
                {
                    ...
                    "all_timeout_str": str(timedelta(seconds=all_timeout))
                    if all_timeout
                    else None,
        ...
        self.record_stats({"runs": rendered_placeholders})
